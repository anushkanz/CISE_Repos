import React, { useState, useEffect, ChangeEvent, FormEvent, ChangeEventHandler } from
'react';
...
useEffect(() => {
fetch(`http://localhost:8082/api/books/${id}`).then((res) => {
    return res.json();
    })
    .then((json) => {
    setBook(json);
    ...
    <input
    type='text'
    placeholder='Published Date'name='published_date'
    className='form-control'
